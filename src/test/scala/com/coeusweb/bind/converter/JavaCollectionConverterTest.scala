...
class JavaCollectionConverterTest {
  
  val locale: Locale = null
  val conv = JavaCollectionConverter.forHashSet(new SimpleIntConverter)
  ...
  def change_separator() {
    val conv = JavaCollectionConverter.forHashSet(new SimpleIntConverter, separator=" ")
    assertEquals(set, conv.parse("1 2", locale))
    assertEquals(set, conv.parse("1  2  ", locale))
  ...
  def do_not_append_space_when_formatting() {
    val conv = JavaCollectionConverter.forHashSet(new SimpleIntConverter, appendSpace=false)
    assertEquals("1,2", conv.format(set, locale))
  }
}
